##This is the function that implements the stupid backoff algorithm.

predict <- function(input) {
        ##Testing that input is only characters
        if (!class(input) == "character") {output <- "Please enter only words"} else {
                ...
                ##Split input into separate words, getting length of input.
                ...
                ##The function has different paths depending on number of words in input
                ...
                if (length(split) >= 4) {
                        ...
                        ##Append results to results object
                        results <- rbind(results, fivegram_results)
                        ##If already found 5 results, finish here
                        if (nrow(results) >=5) {output <- results[1:5,]}
                                ...
                                ##remove NAs from results object
